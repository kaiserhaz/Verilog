...
//	    'h11 : next_data_r = values_q[0];
//	    'h12 : next_data_r = values_q[1];
//	    'h13 : next_data_r = values_q[2];
//	    'h14 : next_data_r = values_q[3];
//	    'h20 : next_data_r = status_val;
   ...
   assign 	      data_in_valid   = req && gnt && write && addr == 0'h21; // was shift_en
   assign             shift_en = data_in_valid && config_val[0]; // new shit_en is now dependent on configuration
   
   // Config Register
   always @(posedge clk)
     if ( ! rst_n )
       config_val <= 0; // Reset value
     else
       if ( config_w )
	 config_val <= data_w;
   ...
   assign out_valid = shift_en && (nr_tap >= DEPTH-1);
